...
sudo yum install -y openstack-heat-common openstack-heat-api openstack-heat-api-cfn openstack-heat-engine python3-pip
...
sudo cp /usr/local/lib/python3.6/site-packages/heat_dashboard/enabled/_[1-9]*.py \
...
STARTTIME=$(date +%s)
sudo systemctl enable --now openstack-heat-api openstack-heat-api-cfn openstack-heat-engine
sudo systemctl restart httpd

sudo firewall-cmd --add-port={8000/tcp,8004/tcp} --permanent
